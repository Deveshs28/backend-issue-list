  ...
  let validateUserInput = () => {
    return new Promise((resolve, reject) => {
      ...
      if (req.body.email) {
        ...
        logger.error(
          "Field missing error during social login",
          ...
          if (err) {
            ...
            newUser.save((err, newUser) => {
              if (err) {
                ...
                let user = {
                  userId: newUser.toObject().userId,
                  userType: newUser.toObject().userType,
            ...
            if (retrievedUserDetails.userType === "Social") {
              let user = {
                userId: retrievedUserDetails.userId,
                userType: retrievedUserDetails.userType,
                firstName: retrievedUserDetails.firstName,
                lastName: retrievedUserDetails.lastName,
              ...
              logger.error(
                "User already registered via email",
                "userController: createUser",
                10
              );
              let apiResponse = response.generate(
